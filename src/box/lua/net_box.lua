...
function remote_methods:stream(stream_id)
    ...
    local function stream_space(sp, stream_id)
        local space = setmetatable (
            { _stream_id = stream_id },
            { __index = sp, __metatable = false }
        )
        return space
    end

    local function stream_space_index(idx, stream_id)
        local index = setmetatable (
            { _stream_id = stream_id },
            { __index = idx, __metatable = false }
        )
        return index
    end

    local function prepare_stream_spaces_and_indices(connection, stream, stream_id)
        if connection.space then
            stream.space = {}
            for space_id, space in pairs(connection.space) do
                stream.space[space_id] = stream_space(space, stream_id)
                if stream.space[space_id].index then
                    local indices = {}
                    for index_id, index in pairs(stream.space[space_id].index) do
                        indices[index_id] = stream_space_index(index, stream_id)
                    end
    ...
    local stream = setmetatable(
        { _stream_id = stream_id, space = {}, _connection = self },
        {
            __index = self,
            __serialize = remote_serialize, __metatable = false
        }
    )
    ...
    function stream:timeout(timeout)
        check_remote_arg(self, 'timeout')
        return self._connection:timeout(timeout)
    end
    function stream:stream()
        return self
    end

    stream._prepare_stream_spaces_and_indices = prepare_stream_spaces_and_indices;
...
function remote_methods:reload_schema()
    ...
    for stream_id, stream in pairs(self._streams) do
        stream._prepare_stream_spaces_and_indices(self, stream, stream_id)
    end
end
